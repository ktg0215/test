...
class Tem(generic.TemplateView):
...
class Shoplist(generic.ListView):
    model = User
    template_name = 'Shop_list'
    # def get_queryset(self):
    #     return Book.objects.filter(Shops__shop='夏目漱石')
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
...
def register_user(request):
    ...
    shop_form = ShopsForm(request.POST or None)
    userdata_form =UserDataForm(request.POST or None)
    if request.method == "POST" and user_form.is_valid() and shop_form.is_valid() and userdata_form.is_valid():
